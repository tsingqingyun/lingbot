...
#     cross_seq_ids: ClassVar[torch.Tensor] = None
#     cross_text_seq_ids: ClassVar[torch.Tensor] = None
#     def __init__(
#         self, 
#         is_cross=False,
#     ) -> None:
...
#             raise RuntimeError(f"block_mask is None, is_cross={self.is_cross}")

#         if self.is_cross:
#             if block_mask.shape[-2] != q_len or block_mask.shape[-1] != kv_len:
#                 print(f"[FlexAttn] rebuilding cross mask: old={block_mask.shape}, new=({q_len}, {kv_len})")
#                 seq_ids = FlexAttnFunc.cross_seq_ids[:q_len]
#                 text_seq_ids = FlexAttnFunc.cross_text_seq_ids[:kv_len]
#                 mask_mod_cross = FlexAttnFunc._get_cross_mask_mod(seq_ids, text_seq_ids)
#                 block_mask = FlexAttnFunc.compiled_create_block_mask(
#                     mask_mod_cross, 1, 1, q_len, kv_len, device=q_varlen.device, _compile=True
#                 )
#         else:
#             if block_mask.shape[-2] != q_len or block_mask.shape[-1] != kv_len:
#                 print(f"[FlexAttn] adjusting self mask: old={block_mask.shape}, new=({q_len}, {kv_len})")
...
#         return x_out
#     def build_cross_attn_additive_mask(
#     encoder_attention_mask: torch.Tensor,
#     query_len: int,
#     dtype: torch.dtype,
#     ) -> torch.Tensor:
#         """
#         encoder_attention_mask: [B, K], 1/True=valid, 0/False=pad
#         return: [B, 1, Q, K], valid=0, invalid=-inf
#         """
#         if encoder_attention_mask.dtype != torch.bool:
#             valid = encoder_attention_mask > 0
#         else:
#             valid = encoder_attention_mask

#         invalid = ~valid  # [B, K]
#         neg_inf = torch.finfo(dtype).min

#         attn_mask = torch.zeros(
#             (valid.shape[0], 1, query_len, valid.shape[1]),
#             device=encoder_attention_mask.device,
#             dtype=dtype,
#         )
#         attn_mask = attn_mask.masked_fill(invalid[:, None, None, :], neg_inf)
#         return attn_mask
#     @staticmethod
#     @torch.no_grad()
#     def init_mask(
...
#         # seq_ids_dev = seq_ids.long().to(device)
#         # text_seq_ids = torch.arange(B)[:, None].expand(-1, text_len).flatten()
#         # text_seq_ids_dev = text_seq_ids.long().to(device)

#         # FlexAttnFunc.cross_seq_ids = seq_ids_dev
#         # FlexAttnFunc.cross_text_seq_ids = text_seq_ids_dev

#         # mask_mod_cross = FlexAttnFunc._get_cross_mask_mod(seq_ids_dev, text_seq_ids_dev)
#         # block_mask_cross = FlexAttnFunc.compiled_create_block_mask(
#         #     mask_mod_cross, 1, 1, len(seq_ids_dev), len(text_seq_ids_dev), device=device, _compile=True
#         # )
...
#             return (seq_ids_dev[q_idx] == text_seq_ids_dev[kv_idx]) & text_valid_dev[kv_idx]
#         return mask_mod
#         # def seq_mask(
...
class FlexAttnFunc(nn.Module):
    ...
    # 只保留 self-attn 用的静态 mask / seq_ids
    attention_mask: ClassVar[BlockMask] = None
    seq_ids: ClassVar[torch.Tensor] = None
    ...
    def forward(
        ...
        if self.is_cross:
            raise RuntimeError("FlexAttnFunc should not be used for cross-attention anymore.")
        ...
        if block_mask is None:
            raise RuntimeError("self-attention block_mask is None")
    ...
    @staticmethod
    def build_batched_cross_attn_additive_mask(
        encoder_attention_mask: torch.Tensor,
        query_len: int,
        dtype: torch.dtype,
    ) -> torch.Tensor:
    ...
    @staticmethod
    def build_flattened_cross_attn_additive_mask(
        ...
        """
        训练态：
        - query 是 [1, total_q, C]，把 batch 维展平了
        - text 也是 [1, B*K, C]
        - 需要根据每个 query token 属于哪个样本，去决定它只能看哪个样本的 text token

        seq_ids: [total_q_max]，其中 pad 位置是 -1
        text_mask: [B, K]，True/1 表示有效 token
        return: [1, 1, q_len, B*K]
        """
        seq_ids = seq_ids[:q_len]  # [Q]
        if text_mask.dtype != torch.bool:
            text_valid = text_mask > 0
        else:
            text_valid = text_mask

        B, K = text_valid.shape
        device = text_valid.device

        text_seq_ids = torch.arange(B, device=device)[:, None].expand(B, K).reshape(-1)   # [B*K]
        text_valid_flat = text_valid.reshape(-1)                                            # [B*K]

        same_batch = seq_ids[:, None] == text_seq_ids[None, :]                             # [Q, B*K]
        ...
        attn_mask = attn_mask.masked_fill(valid[None, None, :, :], 0)
        return attn_mask
    ...
    def init_mask(
        ...
        FlexAttnFunc.attention_mask = block_mask
        FlexAttnFunc.seq_ids = seq_ids.long().to(device)
...
class WanAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        if attn_mode == 'torch':
            ...
            # self-attn 继续 flex；cross-attn 改用 torch SDPA + 动态 mask
            if cross_attention_dim_head is None:
                self.attn_op = FlexAttnFunc(False)
            else:
                self.attn_op = custom_sdpa
        else:
            raise ValueError(
    ...
    def forward(
        ...
        # self-attn (flex) / cross-attn (torch sdpa)
        if isinstance(self.attn_op, FlexAttnFunc):
            hidden_states = self.attn_op(query, key, value)
        else:
            if attention_mask is not None:
                # flash_attn_func 不支持这里的 additive mask，强制 fallback 到 sdpa
                hidden_states = custom_sdpa(query, key, value, attn_mask=attention_mask)
            else:
                if self.attn_op is flash_attn_func:
                    hidden_states = self.attn_op(query, key, value)
                else:
                    hidden_states = self.attn_op(query, key, value)
...
class WanTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def forward_train(self, input_dict):
        ...
        use_cross_mask = True   # 改成 True 就启用新版 cross-attention mask；False 就关闭做对比实验
        ...
        if use_cross_mask and ('text_mask' in latent_dict):
            ...
            cross_attention_mask = FlexAttnFunc.build_flattened_cross_attn_additive_mask(
                ...
                text_mask=latent_dict['text_mask'].to(hidden_states.device),
    ...
    def forward(
        ...
        text_hidden_states = self.condition_embedder.text_embedder(input_dict["text_emb"])  # [B, L, C]
        ...
        cross_attention_mask = None
        if 'text_mask' in input_dict:
            cross_attention_mask = FlexAttnFunc.build_batched_cross_attn_additive_mask(
                encoder_attention_mask=input_dict['text_mask'].to(latent_hidden_states.device),
                query_len=latent_hidden_states.shape[1],
